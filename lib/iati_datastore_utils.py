...
def make_api_request(
    ...
    **kwargs,
    ...
    request_kwargs = {"params": params, "headers": headers, **kwargs}
...
def build_recipient_country_query(
    country_codes: List[str], include_transaction_recipients: bool = True
) -> Optional[str]:
    """
    ...
    if include_transaction_recipients:
        transaction_level_query = (
            f"transaction_recipient_country_code:{country_codes_str}"
        )
...
def build_combined_query(
    ...
    if sector_codes:
        ...
        if sector_q:
            query_parts.append(sector_q)
    if humanitarian_plan_codes:
        humanitarian_q = build_humanitarian_scope_query(humanitarian_plan_codes)
        if humanitarian_q:
            query_parts.append(humanitarian_q)
    if recipient_country_codes:
        country_q = build_recipient_country_query(
            recipient_country_codes, include_transaction_recipients
        )
        if country_q:
            query_parts.append(country_q)
    if transaction_types:
        transaction_q = build_transaction_type_query(transaction_types)
        if transaction_q:
            query_parts.append(transaction_q)
    if additional_query_params:
        query_parts.append(f"({additional_query_params})")

    final_query = (
        "*:*" if not query_parts else " AND ".join(f"({part})" for part in query_parts)
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"Found {response_data.get('response', {}).get('numFound', 0)} activities."
        )
    ...
    try:
        ...
        print(
            f"Preview mode: Found {response_data.get('response', {}).get('numFound', 0)} total, showing {len(docs)} activities."
        )
    ...
    try:
        ...
        response_data = response.json()
        print(
            f"Count_only mode: Found {response_data.get('response', {}).get('numFound', 0)} activities. Documents expected: 0, got: {len(response_data.get('response', {}).get('docs', []))}"
        )
