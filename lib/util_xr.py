...
def standardize_xr_data_usd_to_forex(xr_df, conversion_directions):
    ...
    forex_to_usd_currencies = [
        currency for currency, direction in conversion_directions.items()
...
def find_exchange_rates_for_currency(tf_prepared, currency_data, special_rates):
    ...
    for currency in unique_currencies:
        ...
        if currency in special_rates:
            ...
            currency_transactions = tf_with_rates[tf_with_rates["currency"] == currency].copy()
            ...
            tf_with_rates.loc[tf_with_rates["currency"] == currency, "exchange_rate"] = merged["rate"].values
...
def apply_currency_conversions(tf_with_rates):
    ...
    tf_final["transaction_value_usd"] = np.where(
        ...
        tf_final["transaction_value"] / tf_final["exchange_rate"]  # Convert foreign to USD
    )
...
def convert_all_to_usd(tf: pd.DataFrame):
    ...
    tf_with_rates = find_exchange_rates_for_currency(tf_prepared, standardized_xr, special_rates)
...
def validate_known_conversions():
    ...
    test_cases = [
        ...
        {"amount": 100, "currency": "JOD", "rate": 1/JOD_PEGGED_USD, "expected_usd": 100 * JOD_PEGGED_USD},
        {"amount": 100, "currency": "SAR", "rate": 1/SAR_PEGGED_USD, "expected_usd": 100 * SAR_PEGGED_USD},
    ]
    ...
    for case in test_cases:
        ...
        print(f"{case['amount']} {case['currency']} = {calculated_usd:.2f} USD (expected: {case['expected_usd']:.2f})")
        assert abs(calculated_usd - case["expected_usd"]) < 0.01, f"Conversion failed for {case['currency']}"
...
def check_conversion_consistency():
    ...
    for currency, pegged_rate in [("JOD", JOD_PEGGED_USD), ("SAR", SAR_PEGGED_USD)]:
        ...
        print(f"${test_amount} USD -> {foreign_amount:.2f} {currency} -> ${back_to_usd:.2f} USD")
        assert abs(back_to_usd - test_amount) < 0.01, f"Round-trip conversion failed for {currency}"
...
def spot_check_sample_transactions(tf_with_conversions, n_samples=5):
    ...
    sample_df = tf_with_conversions.dropna(subset=["exchange_rate", "transaction_value_usd"]).sample(n=min(n_samples, len(tf_with_conversions)))
    
    for _, row in sample_df.iterrows():
        print(f"\nTransaction: {row.get('iati_identifier', 'Unknown')}")
        print(f"  Original: {row['transaction_value']:.2f} {row['currency']}")
        print(f"  Exchange Rate (1 USD = X {row['currency']}): {row['exchange_rate']:.4f}")
        print(f"  Converted: ${row['transaction_value_usd']:.2f} USD")
        
        if row['currency'] != 'USD':
            manual_calc = row['transaction_value'] / row['exchange_rate']
            print(f"  Manual calculation: {row['transaction_value']:.2f} / {row['exchange_rate']:.4f} = ${manual_calc:.2f}")
...
def compare_old_vs_new_results(tf_old, tf_new, tolerance=0.01):
    ...
    common_ids = set(tf_old.get('iati_identifier', [])) & set(tf_new.get('iati_identifier', []))
    ...
    for tx_id in list(common_ids)[:10]:  # Check first 10 common transactions
        ...
        if pd.notna(old_usd) and pd.notna(new_usd):
            ...
            if diff > tolerance:
                print(f"⚠️  Large difference for {tx_id}: Old=${old_usd:.2f}, New=${new_usd:.2f}, Diff=${diff:.2f}")
            else:
                print(f"✓ {tx_id}: Old=${old_usd:.2f}, New=${new_usd:.2f}, Diff=${diff:.4f}")
