...
class LabelValidator:
    ...
    def display_activity(self, activity: Dict, index: int, total: int):
        ...
        narrative_fields = [
            ...
            if field
            not in ["title_narrative", "description_narrative", "sector_narrative"]
        ]
        ...
        for i, field in enumerate(self.label_fields, 1):
            ...
            if field == "llm_ref_group":
                ...
                options_display = (
                    f"  │ Options: {', '.join(self.valid_target_populations)}"
                )
    ...
    def edit_activity(self, activity: Dict):
        ...
        while True:
            ...
            for i, field in enumerate(self.label_fields, 1):
                ...
                print(
                    f"   {i}. {display_name:20} {value_display} {valid}{options_display}"
                )
...
def quick_stats(input_file: str):
    ...
    if ref_groups:
        print(
            f"Refugee groups: {dict(sorted(ref_groups.items(), key=lambda x: x[1], reverse=True))}"
        )
    ...
    if target_pops:
        print(
            f"Target populations: {dict(sorted(target_pops.items(), key=lambda x: x[1], reverse=True))}"
        )
